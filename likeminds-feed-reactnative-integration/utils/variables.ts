...
export function manipulateString(str: string, action: string) {
    switch (action) {
      ...
      case WordAction.capitalizeAll:
        return str.toUpperCase();
  
      case WordAction.capitalizeWords:
        return str.replace(/\b\w/g, (char) => char.toUpperCase());

      case WordAction.lowerCaseFirst:
        return str.charAt(0).toLowerCase() + str.slice(1);
      
      case WordAction.lowerCaseAll:
        return str.toLowerCase();
      
      case WordAction.lowerCaseWords:
        return str.replace(/\b\w/g, (char) => char.toLowerCase());
  
      default:
        return str; // Return the original string if action is not recognized
    }
}