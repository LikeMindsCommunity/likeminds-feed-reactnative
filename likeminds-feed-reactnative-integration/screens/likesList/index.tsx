  ...
  return (
    <SafeAreaView style={styles.mainContainer}>
      <LMHeader
        ...
        heading={
          customScreenHeader?.heading ? customScreenHeader?.heading : pluralizeOrCapitalize((CommunityConfigs?.getCommunityConfigs("feed_metadata"))?.value?.likeEntityVariable?.entityName ?? "like",WordAction.firstLetterCapitalPlural)
        }
        subHeading={
          customScreenHeader?.subHeading
            ? customScreenHeader?.subHeading
            : totalLikes > 1
            ? `${totalLikes} ${pluralizeOrCapitalize((CommunityConfigs?.getCommunityConfigs("feed_metadata"))?.value?.likeEntityVariable?.entityName ?? "like",WordAction.allSmallPlural)}`
            : `${totalLikes} ${pluralizeOrCapitalize((CommunityConfigs?.getCommunityConfigs("feed_metadata"))?.value?.likeEntityVariable?.entityName ?? "like",WordAction.allSmallSingular)}`
        }
        onBackPress={() => {
          handleScreenBackPressProp
            ? handleScreenBackPressProp()
